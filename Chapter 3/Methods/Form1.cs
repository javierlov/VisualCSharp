	...
	public class Form1 : System.Windows.Forms.Form
	{
		...
		private void calculate_Click(object sender, System.EventArgs e)
		{
			try 
			{
				...
				if (addition.Checked) 
					showResult(addValues(lhs, rhs));
				else if (subtraction.Checked)
					showResult(subtractValues(lhs, rhs));
				else if (multiplication.Checked)
					showResult(multiplyValues(lhs, rhs));
				else if (division.Checked)
					showResult(divideValues(lhs, rhs));
				else if (remainder.Checked)
					showResult(remainderValues(lhs, rhs));
			}
